...
1. **实时捕获**：在 AI 生成代码的瞬间捕获差异，确保数据的及时性和完整性
2. **准确识别**：通过内容哈希匹配算法，即使代码位置发生变化也能正确识别
3. **手动校正**：针对重复代码等无法准确判断的场景，提供手动标记功能，让用户精确控制代码归属
4. **可视化展示**：提供直观的统计和分析界面，帮助开发者理解代码归属
...
![项目列表截图占位](./project-list.jpg)

#### 步骤 3：查看详情

点击项目，进入详情页面：

![文件分析输入截图占位](./detail.jpg)
...
对于系统无法准确判断的代码（如重复代码），可以手动标记：

1. **点击行号**：点击代码行号，该行的归属会在 AI/人工之间切换
2. **观察状态**：
   - 行号前出现 `●`：有未保存的修改
   - 行背景变为黄色：未保存的手动标记
   - 行背景变为蓝色：已保存的手动标记
3. **保存修改**：点击页面顶部的"保存手动标记"按钮

![手动标记操作截图占位](./screenshots/manual-attribution.png)

手动标记会存储在数据库中，并在后续所有分析中优先使用。

#### 步骤 5：Commit 范围分析（高级功能）
...
![Commit 范围分析截图占位](./screenshots/commit-range-analysis.png)

统计信息包括：
- `added`: 新增代码行数
- `deleted`: 删除代码行数
- `ai_added`: AI 新增的代码行数
- `ai_deleted`: AI 删除的代码行数

### 4. 数据查看与导出

所有 AI 生成记录都存储在 `server/ai_stats.db` SQLite 数据库中，可以使用任何 SQLite 工具查看或导出数据，用于进一步分析。

**数据库表结构**：
- `ai_generations`: AI 生成记录（包含压缩的 diff 数据）
- `manual_attributions`: 用户手动标记的归属信息
...
![系统架构图](./ai-stats-architecture.png)
...
- [正确识别] AI 在 1月15日生成过 `return true;`
...
function debounce(func, wait) {  // ← AI 生成的 5 行代码
    let timeout;
...
function debounce(func, wait) {  // ← 保持不变
    let timeout;
    return function(...args) {
        clearTimeout(timeout);
        timeout = setTimeout(() => func(...args), wait);
...
function multiply(a, b) {    // ← AI 添加的新函数
    return a * b;
}
```
